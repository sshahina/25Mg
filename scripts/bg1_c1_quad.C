...

{
	...
	TFile *f[1] = new TFile("../rootfiles/run_386/UNFILTERED/compass_run_386.root");
	f[0]->ls();
	...
	double channels[8] = {413.715,420.112,506.184,145.036,166.211,173.952,227.573,793.645};
	...
	for(int i = 0; i<NFILES; i++)
	{
		...
		t[i]->Project(name,"Energy+rndm()-0.5","Energy>0 && Board==1 && Channel==1");
	//	h[i]->Rebin(2);
	}
	...
	h[1]->Draw("SAME");
	h[0]->GetXaxis()->SetRangeUser(353,375);//Fit range for 1.4 MeV peak
	h[0]->Fit("fitFunc","Br");
	fitFunc->Draw("SAME");
