...

{
	...
	TFile *f[1] = new TFile("../rootfiles/run_435/UNFILTERED/compass_run_435.root");
	f[0]->ls();
	...
	double channels[8] = {348.728,353.738,430.058,122.488,135.845,146.904,193.006,680.018};
	...
	for(int i = 0; i<NFILES; i++)
	{
		...
		t[i]->Project(name,"Energy+rndm()-0.5","Energy>0 && Board==0 && Channel==2");
	//	h[i]->Rebin(2);
	}
	...
	h[1]->Draw("SAME");
	h[0]->GetXaxis()->SetRangeUser(340,370);//Fit range for 1.4 MeV peak
	h[0]->Fit("fitFunc","Br");
	fitFunc->Draw("SAME");
