...
void CreateBgHists()
{
	...
	ifstream ecal("../cal/calDefault_quadratic.txt");
	if(ecal.is_open())
	{
		for(int i = 0; i<Ndetectors;i++)
		{
			...
			t->SetAlias("Esmeared","Energy+rndm()-0.5");
			t->Project(hname,Form("Esmeared*Esmeared*%g+Esmeared*%g+%g",quad[i],gain[i],offset[i]),
