//
// `root-config --cxx --cflags` -O2 -Wall -Wextra -o extractSpectra_new extractSpectra_new.C `root-config --libs`
// ./extractSpectra_new
//
...
void extractSpectra(TString inDirName, TString outFileName)
{
    // const UShort_t chargeBoard   = 1;
    // const UShort_t chargeChannel = 7;
    
    TChain inChain("Data");
    inChain.Add(inDirName + "/compass_run_*.root");
...
int main()
{
	TString inDirName = "../rootfiles/run_324/UNFILTERED";
	// inDirName = ".";
	TString OutFileName = "spectra.root";
	extractSpectra(inDirName,OutFileName);
