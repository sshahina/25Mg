...
void peakFitter(const char *fileName, const char *fileBack, const char *detector,
	...
	else runNum = runNum.substr(78,3);

	string detNum = detector;

	c0->SaveAs(Form("mkdir -p Yields/P1/run%s/det_%s_Fit.png",runNum.c_str(),detNum.c_str()));
	c0->SaveAs(Form("mkdir -p Yields/P1/det-%i/run%s_Fit.png",detLoop,runNum.c_str()));
...
void mg25Yields(){
	...
	for(int i=324;i<upToRun;i++){
		...
		for(int j=0;j<13;j++){ // 13
			...
			files = Form("/afs/crc.nd.edu/group/nsl/ast/projects/25Mgang/n1analysis/scripts_new/runs/run%d.root",i);
